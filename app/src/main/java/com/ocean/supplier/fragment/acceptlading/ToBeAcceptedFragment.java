 ...
 * 待受理
 */
public class ToBeAcceptedFragment extends BaseFragment {

    @BindView(R.id.rv_loading_type)
    RecyclerView rvLoadingType;
    @BindView(R.id.sv_list)
    SpringView svList;
    private ToBeAcceptAdapter adapter;

    @Override
    protected int attachLayoutRes() {
        return R.layout.fragment_tobe_accept;
    }

    @Override
    protected void initViews() {
        initSpringViewStyle();
        adapter = new ToBeAcceptAdapter(getActivity());
        getData();
    }
    ...
    private void getData() {
        HttpUtil.createRequest(TAG, BaseUrl.getInstence().deliveryBillList()).deliveryBillList(PreferenceUtils.getInstance().getUserToken(),2+"",page+"").enqueue(new Callback<ApiResponse<DeliveryList>>() {
            @Override
            public void onResponse(Call<ApiResponse<DeliveryList>> call, Response<ApiResponse<DeliveryList>> response) {
                ...
                if (response.body() != null) {
                    if (response.body().getCode() == 1) {
                        ...
                        adapter.setOnItemClickLitener(new ToBeAcceptAdapter.OnItemClickLitener() {
                            @Override
                            public void onItemClick(View view, int position) {
                                ToBeAcceptDetailsActivity.actionStart(getActivity(),listBeans.get(position).getSdl_id(),listBeans.get(position).getSdlv_id(),"0");
                            }
                        });
                    } else {
